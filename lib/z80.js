 ...
 */
	...
	parseOpcode: function(s) {
		...
		var indexes = function(par) {
			var disp,prefix;
			var idx = par.substr(0,4).toUpperCase();
			if (idx=='(IX+') {
			...
			if (!s.params) {
				...
				if (s.opcode == 'LD') {
					...
					if (s.bytes==0) {
						...
						if (reg1>=0 && !link2) {
							s.bytes = 3;
							lens = [01 + (reg1<<4), function(vars){return Parser.evaluate(par2,vars);}, null];
							//return s;
						}
						if (reg1>=0 && link2) {
