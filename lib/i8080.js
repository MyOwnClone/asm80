...
var I8080 = {
	...
	"lens": {
		...
		'RB': function(op, params) {
			var reg = this.R(params[0]);
			if (reg<0) throw "Unknown register "+params[0];
			reg <<=3 ;
			return [reg | op, function(vars){return Parser.evaluate(params[1],vars);}];
		},
		'B': function(op, params) {
			return [op, function(vars){return Parser.evaluate(params[0],vars);}];
		},
		'RR': function(op, params) {
			var reg = this.R(params[0]);
			if (reg<0) throw "Unknown register "+params[0];
			reg <<=3 ;
			return [reg | op];
		},
		'RR0': function(op, params) {
			var reg = this.R(params[0]);
			if (reg<0) throw "Unknown register "+params[0];
			return [reg | op];
		},
		'RRR': function(op, params) {
