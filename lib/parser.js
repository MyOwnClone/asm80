...
var Parser = (function (scope) {
	...
	function Parser() {
		...
		this.ops1 = {
			...
			"msb": function(x){Math.floor(x/256);},
			"sin": Math.sin,
			"cos": Math.cos,
			"tan": Math.tan,
			"asin": Math.asin,
		...
		isNumber: function () {
			...
			while (this.pos < this.expression.length) {
				...
				if ((code >= 48 && code <= 57) || code === 46 ||
					...
					if (str[0]==='%') {
						if (str.length<2) {this.pos = bakpos; return false;}
						strx = str.substr(1);
						base = 2;
					}

					//console.log(strx,base);

					this.tokennumber = parseInt(strx,base);
					r = true;
				}
				else {
					break;
			...
			if (strx==='0x') {this.pos = bakpos; return false;}
			return r;
		},
